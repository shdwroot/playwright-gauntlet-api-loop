...
Replace the URL and context path with your own API inputs. Testing does not require access to the application source. Add `--watch` for continuous context maintenance and, separately, `--workflow source-repair --source auto` when you want source repair in a supported local deployment.
...
For a complete local deployment exercise, use the [RESTaurant setup and live example](../examples/restaurant/README.md). Its selected workflow persists after onboarding; pass `--workflow tests-only` to disable source editing while retaining repair and fixture configuration.
