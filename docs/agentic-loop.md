...
Discovery splits source text into line-preserving slices, capped by `discovery.maxAgentInputCharacters` (default 200,000) and the complete-call limit. Builder and verifier work in bounded batches; lead, healer and critic receive focused summaries/evidence. Repeated schemas are shared within each input, while full artifacts and immutable proof checks remain authoritative. Defaults are 160,000 complete-context characters per call, 3,000,000 cumulative characters and 60 calls per run, 8 criteria per batch and 8,000 output tokens per call. Configure these under `agents`; see [bounded context and its limits](configuration.md#bounded-agent-context). Oversized atomic units and exhausted budgets leave explicit gaps, never synthetic passes. Partial verifier results and batch statuses are retained. `discovery.maxCandidates` and `maxCandidatesPerOperation` still bound proposals and generated cases.
