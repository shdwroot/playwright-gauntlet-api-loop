...
Assess only isolationUnitIds assigned in this batch, using the schema's isolation object; other included units may be setup/proof context already reviewed in another batch. Isolated means independent of other test units, with explicit setup and owned cleanup or configured reset hooks where needed. Workflow-internal dependencies are valid when established within that workflow. Serial order alone is not isolation. Stateless read-only health checks need no reset; configured fixture capabilities may establish preconditions, but do not prove the API behavior under test.
...
semantic batches contain only assigned obligations and their linked passing test units. Review each assigned obligation once. isolation batches contain remaining test units and no semantic obligations. reconciliation batches contain unverifiedObligations and freshly verifiedReplacements; compare their complete criteria and propose mappings only between those supplied IDs. Current explicit requirement gaps are not discarded as duplicates. The runtime merges batches, validates every proof against full execution, and retains unreviewed or failed batches as gaps. Do not manufacture a cross-batch proof or assume unseen replacement criteria.
